...
class MilestoneService:
    def __init__(self) -> None: self.rewards = RewardService()
    ...
    async def apply_sale(self, session: AsyncSession, *, organization_id: UUID, customer_id: UUID, order_id: UUID, category_counts: dict[str, int]) -> MilestoneResult:
        ...
        for code, count in sorted(category_counts.items()):
            ...
            if counter is None:
                counter = CustomerCategoryCounter(organization_id=organization_id, customer_id=customer_id, category_id=category.id, lifetime_count=0, net_count=0); session.add(counter); await session.flush()
            before = counter.net_count; counter.lifetime_count += count; counter.net_count += count; counter.version += 1
            ...
            for rule in rules:
                first = before // rule.threshold_count + 1; last = counter.net_count // rule.threshold_count
                if not rule.repeatable: first, last = 1, min(last, 1)
                for milestone_number in range(first, last + 1):
                        ...
                        if existing.revoked:
                            ...
                            if reward is not None and reward.status == RewardStatus.REVOKED.value and reward.consumed_at is None:
                                ...
                                if reward.valid_until is None or reward.valid_until > now:
                                    reward.status = RewardStatus.ACTIVE.value if reward.valid_from <= now else RewardStatus.ISSUED.value; reward.quantity_remaining = 1; reward.revoked_at = None; existing.revoked = False; issued.append(reward.id)
                        continue
                    ...
                    session.add(MilestoneIssuance(organization_id=organization_id, customer_id=customer_id, rule_id=rule.id, rule_version=rule.config_version, threshold_count_snapshot=rule.threshold_count, repeatable_snapshot=rule.repeatable, milestone_number=milestone_number, customer_reward_id=reward.id, source_order_id=order_id)); issued.append(reward.id)
        await session.flush(); return MilestoneResult(tuple(issued))
    ...
    async def apply_refund(self, session: AsyncSession, *, organization_id: UUID, customer_id: UUID, order_id: UUID, category_counts: dict[str, int]) -> None:
        del order_id; now = datetime.now(timezone.utc)
        for code, count in sorted(category_counts.items()):
            if count <= 0: continue
            ...
            counter.net_count = max(0, counter.net_count - count); counter.version += 1
            rule_ids = (await session.scalars(select(MilestoneRewardRule.id).where(MilestoneRewardRule.organization_id == organization_id, MilestoneRewardRule.category_id == category.id))).all()
            if not rule_ids: continue
            issuances = (await session.scalars(select(MilestoneIssuance).where(MilestoneIssuance.organization_id == organization_id, MilestoneIssuance.customer_id == customer_id, MilestoneIssuance.rule_id.in_(rule_ids), MilestoneIssuance.revoked.is_(False)).with_for_update())).all()
            for issuance in issuances:
                if counter.net_count >= issuance.milestone_number * issuance.threshold_count_snapshot: continue
                ...
                if reward is not None and reward.status in {RewardStatus.ACTIVE.value, RewardStatus.ISSUED.value} and reward.quantity_remaining > 0:
                    reward.status = RewardStatus.REVOKED.value; reward.quantity_remaining = 0; reward.revoked_at = now
