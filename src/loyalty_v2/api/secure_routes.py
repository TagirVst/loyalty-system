...
async def _draft_for_principal(session: AsyncSession, draft_id: UUID, organization_id: UUID, location_id: UUID) -> OrderDraft:
    ...
    if draft is None or draft.organization_id != organization_id or draft.location_id != location_id:
        raise HTTPException(status_code=403, detail={"code": "DRAFT_SCOPE_MISMATCH"})
    return draft
...
async def identify(draft_id: UUID, body: IdentifyDraftRequest, session: AsyncSession = Depends(get_session)) -> DraftResponse:
    try:
        async with session.begin():
            principal = await principals.staff(session, staff_session_id=body.staff_session_id)
            principal.require(Permission.SALE_CREATE)
            await _draft_for_principal(session, draft_id, principal.organization_id, principal.location_id)
    ...
    try:
        async with session.begin():
            ...
            if order is None or order.organization_id != principal.organization_id or order.location_id != principal.location_id or order.actor_staff_id != principal.staff_id:
                raise HTTPException(status_code=403, detail={"code": "NOT_OWN_ORDER"})
