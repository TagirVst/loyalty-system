...
async def main() -> None:
    ...
    if not settings.staff_bot_token:
        raise RuntimeError("LOYALTY_STAFF_BOT_TOKEN is required")
    if not settings.organization_id:
        raise RuntimeError("LOYALTY_ORGANIZATION_ID is required")
    bot = Bot(settings.staff_bot_token)
    adapter = StaffBot(bot=bot, sessions=SessionFactory, organization_id=UUID(settings.organization_id))
    try:
