...
class RewardService:
    ...
    async def issue(self, session: AsyncSession, *, organization_id: UUID, customer_id: UUID, reward_definition_id: UUID, source_type: str | None = None, source_id: UUID | None = None, source_key: str | None = None, valid_from: datetime | None = None, valid_until: datetime | None = None, now: datetime | None = None) -> CustomerReward:
        ...
        definition = await session.scalar(select(RewardDefinition).where(RewardDefinition.id == reward_definition_id, RewardDefinition.organization_id == organization_id, RewardDefinition.is_active.is_(True)))
        if definition is None: raise RewardUnavailable("Reward definition is unavailable")
        customer = await session.scalar(select(Customer.id).where(Customer.id == customer_id, Customer.organization_id == organization_id, Customer.is_blocked.is_(False)))
        if customer is None: raise RewardUnavailable("Customer is unavailable")
        if source_key:
            ...
            if existing is not None: raise RewardAlreadyIssued("Reward source was already issued")
    ...
    async def expire_due(self, session: AsyncSession, *, organization_id: UUID, now: datetime | None = None, limit: int = 500) -> int:
        now = now or datetime.now(timezone.utc)
        rewards = (await session.scalars(select(CustomerReward).where(CustomerReward.organization_id == organization_id, CustomerReward.status.in_([RewardStatus.ISSUED.value, RewardStatus.ACTIVE.value]), CustomerReward.valid_until.is_not(None), CustomerReward.valid_until <= now).order_by(CustomerReward.valid_until.asc()).limit(limit).with_for_update(skip_locked=True))).all()
        for reward in rewards: await self.refresh_status(session, reward, now=now)
        await session.flush(); return len(rewards)

    async def activate_due(self, session: AsyncSession, *, organization_id: UUID, now: datetime | None = None, limit: int = 500) -> int:
        now = now or datetime.now(timezone.utc)
        rewards = (await session.scalars(select(CustomerReward).where(CustomerReward.organization_id == organization_id, CustomerReward.status == RewardStatus.ISSUED.value, CustomerReward.valid_from <= now, (CustomerReward.valid_until.is_(None)) | (CustomerReward.valid_until > now)).order_by(CustomerReward.valid_from.asc()).limit(limit).with_for_update(skip_locked=True))).all()
        for reward in rewards: reward.status = RewardStatus.ACTIVE.value
        await session.flush(); return len(rewards)

    async def issue_birthday_if_due(self, session: AsyncSession, *, organization_id: UUID, customer_id: UUID, reward_definition_id: UUID, now: datetime | None = None) -> CustomerReward | None:
        now = now or datetime.now(timezone.utc)
        customer = await session.scalar(select(Customer).where(Customer.id == customer_id, Customer.organization_id == organization_id, Customer.is_blocked.is_(False)).with_for_update())
        if customer is None: raise RewardUnavailable("Customer is unavailable")
        birthday = birthday_in_year(customer.birth_date, now.year); start_date = birthday - timedelta(days=3); end_date = start_date + timedelta(days=7)
        if not (start_date <= now.date() < end_date): return None
