...
class ClientBot:
    ...
    def _routes(self) -> None:
        ...
        async def registration_phone(message: Message, state: FSMContext) -> None:
            if message.contact.user_id != message.from_user.id:
                await message.answer("Нужно отправить свой номер кнопкой ниже.")
                return
        ...
        async def registration_birth(message: Message, state: FSMContext) -> None:
            try:
                day, month, year = map(int, (message.text or "").split("."))
                birth = date(year, month, day)
            except (ValueError, TypeError):
                await message.answer("Не получилось распознать дату. Формат: ДД.ММ.ГГГГ.")
                return
            data = await state.get_data()
            try:
                async with self.sessions.begin() as session:
                    result = await self.customers.register(
                        session, organization_id=self.organization_id, provider="telegram",
                        external_subject=str(message.from_user.id), first_name=data["name"], phone=data["phone"], birth_date=birth,
                    )
            except CustomerAlreadyExists:
                await message.answer("Этот номер или Telegram уже зарегистрирован.")
                return
            await state.clear()
            home = await self.client.home(session, organization_id=self.organization_id, customer_id=result.customer.id)
            await message.answer(self._home_text(home), reply_markup=self.main_keyboard())
        ...
        async def rewards(message: Message) -> None:
            ...
            if not items:
                await message.answer("Активных наград сейчас нет.")
                return
            lines = ["Ваши награды:"]
            for reward, definition in items:
                expiry = reward.valid_until.strftime("%d.%m.%Y") if reward.valid_until else "без срока"
                lines.append(f"• {definition.name} ×{reward.quantity_remaining} — до {expiry}")
        ...
        async def history(message: Message) -> None:
            ...
            for item in items:
                ...
                lines.append(f"• {item.created_at.strftime('%d.%m.%Y')}  {sign}{item.delta} → {item.balance_after}")
            await message.answer("\n".join(lines))
    ...
    def _home_text(home) -> str:
        cashback = home.cashback_basis_points / 100
        lines = [f"{home.customer.first_name}, ваша карта", f"Баллы: {home.balance}", f"Уровень: {home.tier_name}", f"Кэшбэк: {cashback:g}%"]
        if home.next_tier_name and home.next_tier_minimum_spend_minor is not None:
            left = max(0, home.next_tier_minimum_spend_minor - home.qualification_spend_minor) / 100
            lines.append(f"До {home.next_tier_name}: {left:,.0f} ₽".replace(",", " "))
        return "\n".join(lines)
