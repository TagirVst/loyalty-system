...
from loyalty_v2.db.order_models import Order, OrderDraft
from loyalty_v2.db.session import get_session

router = APIRouter(prefix="/api/v2")
staff_auth = StaffAuthService(); principals = PrincipalService(); points = PointsService(); orders = OrderService(); identification = IdentificationService(); refunds = RefundService()


def domain_error(exc: DomainError) -> HTTPException:
    code=exc.code
    if code in {"INVALID_PIN","PIN_LOCKED","STAFF_SESSION_INVALID","TERMINAL_NOT_AUTHORIZED"}: http_status=status.HTTP_401_UNAUTHORIZED
    elif code=="PERMISSION_DENIED": http_status=status.HTTP_403_FORBIDDEN
    elif isinstance(exc,InsufficientPoints): http_status=status.HTTP_409_CONFLICT
    else: http_status=status.HTTP_422_UNPROCESSABLE_ENTITY
    return HTTPException(http_status,detail={"code":code,"message":str(exc)})

async def _draft_for_principal(session:AsyncSession,draft_id:UUID,organization_id:UUID,location_id:UUID)->OrderDraft:
    draft=await session.get(OrderDraft,draft_id)
    if draft is None or draft.organization_id!=organization_id or draft.location_id!=location_id: raise HTTPException(status_code=403,detail={"code":"DRAFT_SCOPE_MISMATCH"})
    return draft
...
async def login(body:StaffLoginRequest,session:AsyncSession=Depends(get_session))->StaffSessionResponse:
    async with session.begin(): attempt=await staff_auth.authenticate_attempt(session,organization_id=body.organization_id,terminal_id=body.terminal_id,pin=body.pin)
    if attempt.error: raise domain_error(attempt.error)
    auth=attempt.auth_session; assert auth is not None
...
async def adjust_points(customer_id:UUID,body:AdjustPointsRequest,session:AsyncSession=Depends(get_session))->PointsEntryResponse:
    try:
        async with session.begin():
            principal=await principals.staff(session,staff_session_id=body.staff_session_id); principal.require(Permission.POINTS_ADJUST)
...
async def create_draft(body:CreateDraftRequest,session:AsyncSession=Depends(get_session))->DraftResponse:
    try:
        async with session.begin():
            principal=await principals.staff(session,staff_session_id=body.staff_session_id); principal.require(Permission.SALE_CREATE)
...
async def identify(draft_id:UUID,body:IdentifyDraftRequest,session:AsyncSession=Depends(get_session))->DraftResponse:
    try:
        async with session.begin():
            principal=await principals.staff(session,staff_session_id=body.staff_session_id); principal.require(Permission.SALE_CREATE); await _draft_for_principal(session,draft_id,principal.organization_id,principal.location_id)
...
async def confirm(draft_id:UUID,body:ConfirmOrderRequest,session:AsyncSession=Depends(get_session))->OrderResponse:
    try:
        async with session.begin():
            principal=await principals.staff(session,staff_session_id=body.staff_session_id); principal.require(Permission.SALE_CONFIRM); await _draft_for_principal(session,draft_id,principal.organization_id,principal.location_id)
...
async def refund_preview(order_id:UUID,body:RefundPreviewRequest,session:AsyncSession=Depends(get_session))->RefundPreviewResponse:
    try:
        async with session.begin():
            ...
            preview=await refunds.preview(session,organization_id=principal.organization_id,order_id=order_id,gross_refund_minor=body.gross_refund_minor,category_counts=body.category_counts)
    except DomainError as exc: raise domain_error(exc) from exc
    return RefundPreviewResponse(gross_refund_minor=preview.gross_refund_minor,paid_refund_minor=preview.paid_refund_minor,restored_points=preview.restored_points,reversed_earned_points=preview.reversed_earned_points,qualification_reversal_minor=preview.qualification_reversal_minor,remaining_gross_minor=preview.remaining_gross_minor,category_counts=preview.category_counts)
...
async def cancel_own(order_id:UUID,body:CancelOwnOrderRequest,session:AsyncSession=Depends(get_session))->RefundResponse:
    try:
        async with session.begin():
            principal=await principals.staff(session,staff_session_id=body.staff_session_id); principal.require(Permission.SALE_CANCEL_OWN)
