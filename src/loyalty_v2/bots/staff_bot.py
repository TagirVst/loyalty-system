...
class StaffBot:
    ...
    def _routes(self) -> None:
        ...
        async def start(message: Message, state: FSMContext) -> None:
            async with self.sessions() as session:
                terminal = await self._terminal(session, message.chat.id)
            if not terminal:
            ...
            await state.set_state(StaffFlow.pin); await state.update_data(terminal_id=str(terminal.id))
            await message.answer("Введите ваш 6-значный PIN.", reply_markup=ReplyKeyboardRemove())
        ...
        async def pin(message: Message, state: FSMContext) -> None:
            ...
            async with self.sessions() as session:
                async with session.begin():
                    attempt = await self.auth.authenticate_attempt(session, organization_id=self.organization_id, terminal_id=terminal_id, pin=(message.text or "").strip())
            if attempt.error:
                await message.answer("Неверный PIN или вход временно заблокирован."); return
            auth_session = attempt.auth_session; assert auth_session is not None
            await state.set_state(None); await state.update_data(staff_session_id=str(auth_session.id))
            await message.answer("Вход выполнен.", reply_markup=self.main_keyboard())
        ...
        async def amount(message: Message, state: FSMContext) -> None:
            try:
                ...
                await message.answer("Введите корректную сумму, например 450 или 450,50."); return
            await state.update_data(gross_amount_minor=minor); await state.set_state(StaffFlow.points)
            await message.answer("Сколько баллов списать? Введите 0, если не списывать.")
        ...
        async def points(message: Message, state: FSMContext) -> None:
            try:
                ...
                await message.answer("Введите целое число баллов, например 0 или 150."); return
            data = await state.get_data()
            async with self.sessions.begin() as session:
                principal = await self._principal(session, state)
                if not principal: await message.answer("Сессия завершена. Выполните /start."); return
                principal.require(Permission.SALE_CREATE)
                draft = await self.orders.create_draft(session, organization_id=principal.organization_id, location_id=principal.location_id, gross_amount_minor=data["gross_amount_minor"], requested_points=requested)
                draft_id = draft.id
            await state.update_data(draft_id=str(draft_id)); await state.set_state(StaffFlow.code)
            await message.answer("Введите 5-значный код клиента.")
        ...
        async def identify(message: Message, state: FSMContext) -> None:
            ...
            try:
                async with self.sessions.begin() as session:
                    ...
                    rewards = await self.clients.rewards(session, organization_id=principal.organization_id, customer_id=draft.customer_id)
                    reward_data = [(str(r.id), d.name) for r, d in rewards]
            except DomainError as exc:
                await message.answer(f"Не удалось применить код: {exc}"); return
            await state.update_data(available_rewards=reward_data, selected_rewards=[]); await state.set_state(StaffFlow.rewards)
            await message.answer("Клиент найден. Выберите награды или сразу рассчитайте.", reply_markup=self._reward_keyboard(reward_data, set()))
        ...
        async def toggle_reward(callback: CallbackQuery, state: FSMContext) -> None:
            ...
            if reward_id in selected: selected.remove(reward_id)
            else: selected.add(reward_id)
            await state.update_data(selected_rewards=list(selected))
            await callback.message.edit_reply_markup(reply_markup=self._reward_keyboard(data.get("available_rewards", []), selected)); await callback.answer()
        ...
        async def calculate(callback: CallbackQuery, state: FSMContext) -> None:
            ...
            try:
                async with self.sessions.begin() as session:
                    ...
                    draft.selected_reward_ids = selected; draft.version += 1
                    result = await self.orders.quote(session, organization_id=principal.organization_id, draft_id=draft.id)
                    quote = result.quote
            except DomainError as exc:
                await callback.answer(); await callback.message.answer(f"Не удалось рассчитать: {exc}"); return
            await state.update_data(quote_id=str(quote.id)); await state.set_state(StaffFlow.confirm)
            await callback.message.edit_reply_markup(reply_markup=None)
            await callback.message.answer(self._quote_text(quote), reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Подтвердить продажу", callback_data="confirm_sale")],[InlineKeyboardButton(text="Отмена", callback_data="abort_sale")]])); await callback.answer()
        ...
        async def confirm_sale(callback: CallbackQuery, state: FSMContext) -> None:
            ...
            try:
                async with self.sessions.begin() as session:
                    ...
                    order = await self.orders.confirm(session, organization_id=principal.organization_id, draft_id=draft_id, quote_id=quote_id, idempotency_key=key, actor_staff_id=principal.staff_id)
            except DomainError as exc:
                await callback.answer(); await callback.message.answer(f"Не удалось подтвердить: {exc}"); return
            await state.set_state(None); await state.update_data(last_order_id=str(order.id))
            kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Отменить эту продажу",callback_data=f"cancel:{order.id}")]])
            await callback.message.edit_reply_markup(reply_markup=None); await callback.message.answer(f"Продажа проведена.\nОплачено: {order.paid_amount_minor/100:.2f} ₽\nНачислено баллов: {order.points_earned}",reply_markup=kb); await callback.answer()

        @self.router.callback_query(F.data.startswith("cancel:"))
        async def cancel_request(callback: CallbackQuery, state: FSMContext) -> None:
            await state.update_data(cancel_order_id=callback.data[7:]); await state.set_state(StaffFlow.cancel_reason)
            await callback.message.answer("Укажите причину отмены продажи."); await callback.answer()
        ...
        async def cancel_reason(message: Message, state: FSMContext) -> None:
            ...
            try:
                ...
                await message.answer(f"Не удалось отменить: {exc}"); return
        ...
        async def abort(callback: CallbackQuery, state: FSMContext) -> None:
            await state.set_state(None); await callback.message.edit_reply_markup(reply_markup=None); await callback.message.answer("Продажа не проведена.",reply_markup=self.main_keyboard()); await callback.answer()
    ...
    def _reward_keyboard(items: list[tuple[str,str]], selected: set[str]) -> InlineKeyboardMarkup:
        ...
        for reward_id,name in items:
            mark="✓ " if reward_id in selected else ""
            rows.append([InlineKeyboardButton(text=f"{mark}{name}"[:60],callback_data=f"rw:{reward_id}")])
        rows.append([InlineKeyboardButton(text="Рассчитать",callback_data="calc")])
