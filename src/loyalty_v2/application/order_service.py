...
class IdentificationService:
    ...
    async def attach_to_draft(self, session: AsyncSession, *, organization_id: UUID, draft_id: UUID, code: str) -> OrderDraft:
        now = datetime.now(timezone.utc)
        ident = await session.scalar(select(IdentificationSession).where(
            IdentificationSession.organization_id == organization_id,
            IdentificationSession.code == code,
            IdentificationSession.status == "active",
        ).with_for_update())
        ...
        draft.identification_session_id = ident.id
        draft.version += 1
        await session.flush()
        return draft
...
class OrderService:
    ...
    async def create_draft(self, session: AsyncSession, *, organization_id: UUID, location_id: UUID, gross_amount_minor: int, requested_points: int = 0, currency_code: str = "RUB", selected_reward_ids: list[UUID] | None = None, category_counts: dict[str, int] | None = None) -> OrderDraft:
        ...
        if categories:
            known = set((await session.scalars(select(SaleCategory.code).where(
                SaleCategory.organization_id == organization_id,
                SaleCategory.is_active.is_(True),
                SaleCategory.code.in_(categories.keys()),
            ))).all())
            if known != set(categories):
                raise DraftNotReady("Unknown or inactive sale category")
        draft = OrderDraft(
            organization_id=organization_id,
            ...
            selected_reward_ids=[str(x) for x in (selected_reward_ids or [])],
    ...
    async def _calculation(self, session: AsyncSession, *, organization_id: UUID, draft: OrderDraft, account: PointsAccount, now: datetime):
        policy = await self.policies.resolve(session, organization_id=organization_id, customer_id=draft.customer_id, now=now)
        selected_ids = [UUID(x) for x in draft.selected_reward_ids]
        try:
        ...
        after = max(draft.gross_amount_minor - effects.total_discount_minor, 0)
        max_by_percent = (after * policy.redemption_percent) // (100 * POINT_MINOR_VALUE)
        maximum = min(max_by_percent, account.balance, after // POINT_MINOR_VALUE)
        if draft.requested_points > maximum:
            raise RedemptionLimitExceeded("Requested points exceed current redemption limit")
        redeemed = draft.requested_points
        paid = after - redeemed * POINT_MINOR_VALUE
        base = 0 if redeemed else (paid * policy.effective_tier.cashback_basis_points) // 1_000_000
        earned = base * effects.cashback_multiplier
        snapshot = {
            ...
            "effective_tier_id": str(policy.effective_tier.id),
            "automatic_tier_id": str(policy.automatic_tier.id),
            "tier_override_id": str(policy.tier_override_id) if policy.tier_override_id else None,
            "redemption_percent": policy.redemption_percent,
            "redemption_override_id": str(policy.redemption_override_id) if policy.redemption_override_id else None,
            "inactivity_steps": policy.inactivity_steps,
        }
    ...
    async def quote(self, session: AsyncSession, *, organization_id: UUID, draft_id: UUID) -> QuoteResult:
        ...
        state = await session.scalar(select(CustomerLoyaltyState).where(CustomerLoyaltyState.customer_id == draft.customer_id))
        account = await session.scalar(select(PointsAccount).where(PointsAccount.customer_id == draft.customer_id))
        if customer is None or customer.is_blocked or state is None or account is None:
            raise DraftNotReady("Customer loyalty state is unavailable")
        now = datetime.now(timezone.utc)
        policy, _, after, maximum, redeemed, paid, earned, snapshot, _ = await self._calculation(session, organization_id=organization_id, draft=draft, account=account, now=now)
        qualification = after
        potential = await self.tiers.tier_for_spend(session, organization_id, state.qualification_spend_minor + qualification)
        quote = OrderQuote(
            organization_id=organization_id,
            draft_id=draft.id,
            draft_version=draft.version,
            tier_id=policy.effective_tier.id,
            gross_amount_minor=draft.gross_amount_minor,
            amount_after_rewards_minor=after,
            max_redeemable_points=maximum,
            redeemed_points=redeemed,
            paid_amount_minor=paid,
            points_to_earn=earned,
            qualification_amount_minor=qualification,
            potential_tier_id=potential.id,
            loyalty_effects_snapshot=snapshot,
            category_counts_snapshot=dict(draft.category_counts),
            expires_at=now + timedelta(seconds=QUOTE_TTL_SECONDS),
        )
        session.add(quote)
        await session.flush()
        return QuoteResult(quote, account.balance)
    ...
    async def confirm(self, session: AsyncSession, *, organization_id: UUID, draft_id: UUID, quote_id: UUID, idempotency_key: str, actor_staff_id: UUID | None = None) -> Order:
        ...
        if quote.draft_version != draft.version or quote.category_counts_snapshot != draft.category_counts:
        ...
        if ident is None or ident.status != "active" or ident.expires_at <= now:
            raise IdentificationExpired("Identification session is no longer valid")
        customer = await session.scalar(select(Customer).where(Customer.id == draft.customer_id, Customer.organization_id == organization_id).with_for_update())
        state = await session.scalar(select(CustomerLoyaltyState).where(CustomerLoyaltyState.customer_id == draft.customer_id).with_for_update())
        if customer is None or customer.is_blocked or state is None:
        ...
        policy, _, after, maximum, redeemed, paid, earned, snapshot, selected_ids = await self._calculation(session, organization_id=organization_id, draft=draft, account=account, now=now)
        if snapshot != quote.loyalty_effects_snapshot or paid != quote.paid_amount_minor or earned != quote.points_to_earn or maximum != quote.max_redeemable_points:
            raise QuoteStale("Loyalty conditions changed after quote")
        qualification = after
        new_qualification = state.qualification_spend_minor + qualification
        ...
        order = Order(
            ...
            qualification_amount_minor=qualification,
            tier_before_id=policy.effective_tier.id,
        ...
        await self.milestones.apply_sale(
            session,
            organization_id=organization_id,
            customer_id=draft.customer_id,
            order_id=order.id,
            category_counts=dict(draft.category_counts),
        )
        ...
        ident.status = "consumed"
        ident.consumed_at = now
        await session.flush()
        return order
